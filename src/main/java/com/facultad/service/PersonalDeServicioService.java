...
public class PersonalDeServicioService {
    ...
    public ResponseEntity buscarPersonalServicio(String dni) {
        if (empleadosRepository.getListaEmpleados().containsKey(dni) && empleadosRepository.getListaEmpleados().get(dni).getCargo().equals(CargoEnum.PERSONAL_DE_SERVICIO)) {
    ...
    public ResponseEntity modificarEmpleadoServicio(String dni, PersonalDeServicio personalDeServicio) {
        if (personalDeServicio == null || !empleadosRepository.getListaEmpleados().get(dni)
